...
const CLOZE_I18N = {
  en: {
    ...
    legendTitle: "Legend",
    legendNodeHeader: "<strong>Node header</strong> — internal node/leaf title area (green).",
    ...
    legendAttrName: "<strong>Attribute name</strong> — attribute used to split at a node (blue band at the bottom of a node).",
    legendLeafLabel: "<strong>Label value</strong> — class predicted at a leaf (yellow band at the bottom of a leaf).",
    legendBranches: "<strong>Branches & labels</strong> — arrows connect parent→child; the text along the arrow is the attribute<br>value that follows that branch.",
  ...
  es: {
    title: "Ejercicio Árbol ID3",
    leadFull: `Rellena los valores que faltan usando el árbol de decisión y la tabla de datos. 
      Usa el algoritmo <strong>ID3</strong> 
      (<a href="%%LINK_ID3%%" target="_blank" rel="noopener">¿qué es ID3?</a>) 
      y el criterio de <strong>Ganancia de Información</strong> 
      (<a href="%%LINK_IG%%" target="_blank" rel="noopener">más info</a>).`,
    ...
    questionsNote: "Responde usando el formato Cloze de Moodle. Las respuestas numéricas aceptan la tolerancia indicada.",
    legendTitle: "Leyenda",
    legendNodeHeader: "<strong>Cabecera del nodo</strong> — zona de título del nodo/hoja (verde).",
    legendNodeDetailsN: "número de ejemplos de entrenamiento que alcanzan este nodo.",
    legendNodeDetailsIG: "Information Gain calculado en este nodo (criterio de ID3).",
    legendAttrName: "<strong>Nombre del atributo</strong> — atributo usado para dividir en un nodo (banda azul inferior en nodos).",
    legendLeafLabel: "<strong>Valor de etiqueta</strong> — clase predicha en una hoja (banda amarilla inferior en hojas).",
    legendBranches: "<strong>Ramas y etiquetas</strong> — las flechas conectan padre→hijo; el texto a lo largo de la flecha es el<br>valor del atributo que sigue esa rama.",
    igEntropy: "IG/Entropía",
    attribute: "Atributo",
    leafLabel: "Etiqueta de hoja",
    at: "en",
    legendIdxNote_es: '<br><strong>Nota:</strong> los números entre corchetes (p. ej. [1], [2])<br>indican las posiciones que corresponden a las preguntas Cloze siguientes.',
  }
};
...
function buildTreeLegendSvg_DOM(lang) {
  ...
  svg.setAttribute("width", "900");           // un poco más ancho
  ...
  // Línea con posible <strong>…</strong> y sin/ con <br>
  function addLegendLine(x, y, html, opts={}) {
    // soportar saltos de línea con <br> en la traducción
    ...
    lines.forEach((line, idx) => {
      ...
      appendSpan(pre,    null);
      appendSpan(strong, '600');   // 👈 aquí va la negrita
  ...
  // Node header  "<strong>Node header</strong> — …"
  ...
  // Attribute name "<strong>Attribute name</strong> — …"
  addBox(20, y-12, "#ADD8E6");
  addLegendLine(50, y, t('legendAttrName', lang));
  y += 24;

  // Leaf label "<strong>Label value</strong> — …"
  ...
  // Nota: PERMITE <br> en la traducción para partir líneas largas
  // Ejemplo en inglés:
...
function buildFeedbackHtml(targetsSorted, lang) {
  ...
  if (!withHints.length) {
    // Si no hay hints activos, devolver cadena vacía
    return '';
  }
  ...
  const lis = withHints.map(t => {
    ...
    // Envolvemos el texto (custom o default) con el índice [i]
...
function defaultFeedbackBaseFor(nodeId, key, lang = getLang()) {
  ...
  if (key === 'attr') {
    return isEs
      ? `(atributo en ${nodeId}): elige el atributo que maximiza la ganancia de información (IG) entre los atributos disponibles en ese nodo.`
  ...
  // metric (IG/Entropy)
  return isEs
    ? `(IG/Entropía en ${nodeId}): calcula la entropía de la distribución de clases en ese nodo y aplica la fórmula de ganancia de información.`
...
function getBranchTextFromDefs(symId) {
  ...
  if (!sym) return '';
  // En tus SVGs suele estar en <tspan id="branchValue…">, pero cubrimos fallback
...
function cloneNeededDefs(srcSvg, symIds) {
  ...
  if (srcDefs) {
    ...
    srcDefs.querySelectorAll('marker').forEach(m => {
      ...
      mk.setAttribute('markerWidth',  '2');  // ← antes 5/6
    ...
    // symbols de nodos visibles (como ya hacías)
...
function rebuildPreviewSvg(highlightId = null) {
  ...
  // menos padding para árboles pequeños
  ...
  // tamaño de la caja destino (interior del preview)
  const dstW = Math.max(100, Math.floor(box.clientWidth  - 12)); // ≈ padding visual
  ...
  edges.forEach(([p,c], i) => {
    ...
    const margin = Math.max(6, Math.min(12, len * 0.06));  // 6–12 px
    ...
    if (label) {
      ...
      // halo blanco para legibilidad + ligera separación de la línea
  ...
  nodes.forEach(n => {
    ...
    // para poder hacer highlight rápido
...
function getNodesBBoxWithDynamicPadding() {
  ...
  // Padding adaptativo:
  // 1–2 nodos: casi sin padding → ¡gran escala!
  // 3–6 nodos: moderado
  // 7+ nodos: como antes
  let k, minPad, maxPad;
  if (count <= 2) {
...
function buildDataTableSection(lang = getLang()) {
  ...
  if (sampleBodyCell) {
    const cs = sampleBodyCell.style; // ya está inlined
...
function openFeedbackEditor(inputEl, defaultText, label) {
  ...
  titleEl.textContent = label || (lang === 'es' ? 'Editar retroalimentación' : 'Edit feedback');
...
function getDefaultTheoryQuestions(lang = getLang()) {
  ...
  // Intentamos sacar info real del dataset y del árbol actual
  ...
  if (!stats || !cols.length || !stats.rootAttr || !stats.usedAttributes.length || !stats.numLeaves) {
    ...
    if (isEs) {
      return [
        '¿Cuál es el atributo con mayor entropía del conjunto de datos? {1:MULTICHOICE_S:%100%outlook#~%-33%temperature#~%-33%humidity#~%-33%windy#}',
        '¿Cuántas instancias del conjunto de datos están siendo tratadas en este paso del algoritmo de construcción del árbol? {1:NUMERICAL:%100%5:0#}',
        '¿A qué nodo se corresponde la información de la tabla que estás viendo? {1:MULTICHOICE_S:%33%Node 1#~%33%Node 2#~%100%Node 3#~%-33%Leaf 1#~%-33%Leaf 2#~%-33%Leaf 3#}',
        '¿Cuántos nodos hoja tendrá la construcción final del árbol de decisión de ID3? {1:NUMERICAL:%100%5:0#}',
        '¿Qué atributos utilizará el árbol de decisión final construido? {1:MULTICHOICE_M:%100%outlook#~%100%humidity#~%-33%temperature#~%-33%windy#}'
      ];
    } else {
      return [
  ...
  if (isEs) {
    return [
      `¿Qué atributo se usa en la raíz del árbol de decisión? ${clozeRoot}`,
      `¿Cuántas instancias del conjunto de datos se están usando en este árbol de decisión? ${clozeRows}`,
      `¿Cuántos nodos hoja tiene el árbol de decisión final de ID3 para este conjunto de datos? ${clozeLeaves}`,
      `¿Qué atributos aparecen en algún nodo interno del árbol de decisión? ${clozeAttrsSet}`
    ];
  } else {
    return [
...
function renderTheoryQuestions() {
  ...
  if (!THEORY_QUESTIONS.length) {
    ...
    info.textContent = isEs
      ? 'Todavía no hay preguntas. Pulsa "Add question" para crear una.'
...
function exportClozeXmlWithConfig(CONFIG) {
  ...
  // ---- clonar SVG del árbol y meter [i] ----
...
${targetsSorted.map(t => {
  ...
  Promise.all([
    ...
    // Reusamos tu builder de página completo y luego nos quedamos solo con el <body>
    ...
    const title = (lang === 'es')
      ? 'Ejercicio ID3 – Árbol de decisión'
      : 'ID3 Tree Exercise';
...
function openClozeModal(){
  ...
  nodes.forEach(n => {
    ...
    targets.forEach(t => {
      ...
      // habilitar / deshabilitar lápiz según checkbox
      function syncHintUi() {
        if (!fbBtn) return;
      ...
      if (fbBtn) {
        fbBtn.addEventListener('click', () => {
          ...
          const label = (lang === 'es')
            ? `Editar retroalimentación (${t.label} – ${n.id})`
            : `Edit feedback (${t.label} – ${n.id})`;
...
function exportTheoryQuestionsXmlFromModel() {
  ...
  if (!svg) {
    alert(lang === 'es'
      ? 'No se ha encontrado el árbol de decisión para exportar.'
  ...
  if (!THEORY_QUESTIONS.length) {
    alert(lang === 'es'
      ? 'No hay preguntas teóricas definidas.'
  ...
  Promise.all([
    ...
    // ----- construir HTML de la pregunta (enunciado + dataset + imágenes + preguntas) -----
    const isEs = (lang === 'es');

    const introHtml = isEs
          ? `
    <p>En este ejercicio analizamos la construcción de un clasificador de árboles de decisión ID3 sobre el conjunto de datos actual. Este conjunto de datos contiene información tabular para tomar una decisión de clasificación.</p>
    <p>En el siguiente gráfico se presenta un estado parcial (step ${stepLabel}) de los recursos de información que utiliza el algoritmo ID3 para la construcción del árbol de decisión. La entrada es la tabla de datos que se muestra a continuación y en la última columna está la clase a predecir. Para la construcción del árbol de decisión, se van creando los nodos aplicando el criterio de ganancia de información calculado con la entropía condicional (H).</p>
    `
          : `
    ...
    const questionName = isEs
        ? `Árbol cuestiones (step ${stepLabel})`
    ...
    const feedbackHtml = isEs
      ? `<p>Recuerda que la ganancia de información se calcula a partir de la entropía condicional y que el algoritmo ID3 selecciona siempre el atributo con mayor IG en cada paso. Fíjate en la tabla de IG/entropías y en cuántas instancias llegan a cada nodo.</p>`
...
function openTheoryModal() {
  ...
  // Igual patrón que en el CLOZE modal: insertar selector en la cabecera
  (function ensureTheoryLangSelector() {
    ...
    if (sel) {
      ...
      sel.onchange = () => {
        setLang(sel.value);
        // No regeneramos las preguntas aquí para no pisar cambios del usuario
      };
    }
  })();
